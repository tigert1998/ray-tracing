...
optional<HitRecord> Triangle::Hit(const Ray &ray, pair<double, double> t_range) const {
	vec3 n = normalize(cross(p_[2] - p_[0], p_[1] - p_[2]));
	double den = dot(ray.direction(), n);
	double num = dot(p_[0] - ray.position(), n);
	double t = num / den;
	if (std::isnan(t) || std::isinf(t)) return none;
	if (t < t_range.first || t > t_range.second) return none;
	vec3 hit_point = ray.position() + (float) t * ray.direction();
	{
		...
		for (int i = 0; i < 3; i++)
			if (dot_values[i] * dot_values[(i + 1) % 3] < 0)
				return none;
	}
	HitRecord record;
	record.t = t;
