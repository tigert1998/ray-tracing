...
PerlinNoise::WeightsType PerlinNoise::CalcWeights(double x, double y, double z) const {
...
PerlinNoise::PerlinNoise(int perm_size): PERM_SIZE(perm_size) {
    if (perm_size != (perm_size & -perm_size)) throw std::invalid_argument("");
...
double PerlinNoise::Grad(int x, int y, int z, double dx, double dy, double dz) const {
    x &= PERM_SIZE - 1;
    y &= PERM_SIZE - 1;
    z &= PERM_SIZE - 1;
...
highp_dvec3 PerlinNoise::DerivativeNoise(double x, double y, double z) const {
    ...
    double dx = x - floor(x), dy = y - floor(y), dz = z - floor(z);
    {
        ...
        for (int i = 0; i < 4; i++)
            dxw[i] = (w[i + 4] - w[i]) * DerivativeF(dx);
        for (int i = 0; i < 2; i++)
    ...
    }
    {
        ...
        for (int i = 0; i < 2; i++)
            dxyw[i] = (xw[i + 2] - xw[i]) * DerivativeF(dy);
        for (int i = 0; i < 1; i++)
            dxyzw[i] = dxyw[i] * (1 - F(dz)) + dxyw[i + 1] * F(dz);
        ans.y = dxyzw[0];
    }
