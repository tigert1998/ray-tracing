...
Camera::Camera(vec3 position, vec3 look_at, vec3 up, double field_of_view, double width_height_aspect, double aperture, double focus_distance) {
    ...
    lower_left_corner_ = position - w_ - u_ * static_cast<float>(half_width) - v_ * static_cast<float>(half_height);
    lens_radius_ = aperture / 2;
}

Camera::Camera(vec3 position, vec3 look_at, vec3 up, double field_of_view, double width_height_aspect) {
...
Camera::Camera(vec3 position, vec3 lower_left_corner, vec3 horizontal, vec3 vertical) {
    is_defocus_camera_ = false;
    position_ = position;
...
vec3 Camera::lower_left_corner() const {
...
Ray Camera::ray_at(double x, double y) const {
    x = (x + 1) / 2;
    y = (y + 1) / 2;
    auto target = lower_left_corner() + static_cast<float>(x) * horizontal() + static_cast<float>(y) * vertical();
    vec3 origin;
...
bool Camera::is_defocus_camera() const {
    return is_defocus_camera_;
}
