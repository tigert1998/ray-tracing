...
Parallelepiped::Parallelepiped(array<vec3, 4> p,
...
optional<HitRecord> Parallelepiped::Hit(const Ray &ray,
                                        pair<double, double> t_range) const {
  return list_.Hit(ray, t_range);
}

void Parallelepiped::AddCorner(const array<vec3, 4> &p,
                               shared_ptr<Material> material_ptr) {
  for (int i = 1; i <= 3; i++) {
    int x = i, y = i + 1 == 4 ? 1 : x + 1;
    list_.list().push_back(make_shared<Parallelogram>(
        array<vec3, 3>({p[0], p[x], p[y]}), material_ptr));
  }
};

Parallelepiped::Parallelepiped(vec3 lengths, shared_ptr<Material> material_ptr,
